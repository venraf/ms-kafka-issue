...
@Service
public class CreateLoggingResponseEventConsumer {

    private final Logger log = new Logger(this.getClass());

    @MyKafkaListener(topics = LOGGING_SERVICE_RESPONSE_AS_VALUE)
    protected void consume(ConsumerRecord<?, InternalEvent> input) {
        log.info(RECEIVED_NEW_EVENT, CREATE_RESPONSE_LOGGING, input.value());
    }
